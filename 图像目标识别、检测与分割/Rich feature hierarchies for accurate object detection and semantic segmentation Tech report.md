...
## 3.达到什么效果？  
    与当前表现最好的算法相比，mAP提升了30%。
## 4.存在什么不足？
    预先提取候选区域，占用较大的磁盘空间；
    每一个region proposal都需要进行特征提取，重复的特征提取带来巨大的计算浪费；
    传统CNN需要固定尺寸的输入图像，warp产生物体拉伸，导致输入CNN的信息丢失；
    实时性比较一般。
    （warp：将原图像的一部分进行缩放，改变其真实尺寸，以达到想要的尺寸比）
...
* 测试时，我们的方法在每张输入图像上生成了约2000张类别独立的region proposal，使用CNN从每个proposal中提取了固定长度的特征向量，然后使用特定目标的线性SVM对每个region进行分类。我们不考虑region的大小，使用放射图像变形的方法来对每个不同形状的region proposal产生一个固定长度的作为CNN输入的特征向量（也就是把不同大小的proposal放到同一个大小）。（这里也是后续进行改进的地方，R-CNN无视候选区域的大小和形状，为了与AlexNet兼容，进行了暴力变换）
* R-CNN overview：
  * 1.input image

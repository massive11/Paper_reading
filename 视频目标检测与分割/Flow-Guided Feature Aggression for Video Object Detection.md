...
#### Detection network
* 我们使用最先进的R-FCN并遵循Deep feature flow中的设计。 在 1024-d 特征图之上，应用了 RPN 子网络和 R-FCN 子网络，它们分别连接到前 512-d 和最后 512-d 特征。 在 RPN 中使用了 9 个锚点（3 个尺度和 3 个纵横比），每张图像产生 300 个建议。 R-FCN 中的位置敏感分数图是 7 × 7 组。
